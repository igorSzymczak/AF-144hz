   ...
   public class SmartGun extends Weapon
   {
      ...
      override protected function shoot() : void
      {
         ...
         while(fireNextTime < g.time)
         {
            ...
            if(burstCurrent > 0)
            {
               fireNextTime = g.time + _loc3_;
            }
            else if(fireNextTime == 0 || lastFire == 0)
            {
               fireNextTime = _loc1_ + _loc3_ - Game.dt;
            }
            else
            {
               fireNextTime += _loc3_;
      ...
      private function fireProjectiles(param1:Number = 0) : void
      {
         ...
         while(_loc9_ < multiNrOfP)
         {
            ...
            _loc2_.course.pos.x = unit.pos.x + Math.cos(unit.rotation + _loc4_ + _loc7_ + param1) * _loc8_;
            _loc2_.course.pos.y = unit.pos.y + Math.sin(unit.rotation + _loc4_ + _loc7_ + param1) * _loc8_;
            _loc2_.course.rotation = unit.rotation + _loc4_ + param1 + (angleVariance - Math.random() * angleVariance * 2);
            if(fireBackwards)
            {
